...
public struct CBORDecoderTransform : InternalDecoderTransform, InternalValueDeserializer {
  ...
  public static func unbox(_ value: CBOR, as type: Bool.Type, decoder: InternalValueDecoder<CBOR, Self>) throws -> Bool? {
    switch value {
    case .boolean(let value): return value
    case .null: return nil
    case let cbor:
      throw DecodingError._typeMismatch(at: decoder.codingPath, expectation: type, reality: cbor)
    }
  }
  ...
  public static func unbox(_ value: CBOR, as type: Int8.Type, decoder: InternalValueDecoder<CBOR, Self>) throws -> Int8? {
    switch value.untagged {
    case .double(let dbl): return try coerce(dbl, at: decoder.codingPath)
    case .float(let flt): return try coerce(flt, at: decoder.codingPath)
    case .unsignedInt(let uint): return try coerce(uint, at: decoder.codingPath)
  ...
  public static func unbox(_ value: CBOR, as type: Int16.Type, decoder: InternalValueDecoder<CBOR, Self>) throws -> Int16? {
    switch value.untagged {
    case .double(let dbl): return try coerce(dbl, at: decoder.codingPath)
    case .float(let flt): return try coerce(flt, at: decoder.codingPath)
    case .unsignedInt(let uint): return try coerce(uint, at: decoder.codingPath)
  ...
  public static func unbox(_ value: CBOR, as type: Int32.Type, decoder: InternalValueDecoder<CBOR, Self>) throws -> Int32? {
    switch value.untagged {
    case .double(let dbl): return try coerce(dbl, at: decoder.codingPath)
    case .float(let flt): return try coerce(flt, at: decoder.codingPath)
    case .unsignedInt(let uint): return try coerce(uint, at: decoder.codingPath)
  ...
  public static func unbox(_ value: CBOR, as type: Int64.Type, decoder: InternalValueDecoder<CBOR, Self>) throws -> Int64? {
    switch value.untagged {
    case .double(let dbl): return try coerce(dbl, at: decoder.codingPath)
    case .float(let flt): return try coerce(flt, at: decoder.codingPath)
    case .unsignedInt(let uint): return try coerce(uint, at: decoder.codingPath)
  ...
  public static func unbox(_ value: CBOR, as type: UInt.Type, decoder: InternalValueDecoder<CBOR, Self>) throws -> UInt? {
    switch value.untagged {
    case .double(let dbl): return try coerce(dbl, at: decoder.codingPath)
    case .float(let flt): return try coerce(flt, at: decoder.codingPath)
    case .unsignedInt(let uint): return try coerce(uint, at: decoder.codingPath)
  ...
  public static func unbox(_ value: CBOR, as type: UInt8.Type, decoder: InternalValueDecoder<CBOR, Self>) throws -> UInt8? {
    switch value.untagged {
    case .double(let dbl): return try coerce(dbl, at: decoder.codingPath)
    case .float(let flt): return try coerce(flt, at: decoder.codingPath)
    case .unsignedInt(let uint): return try coerce(uint, at: decoder.codingPath)
  ...
  public static func unbox(_ value: CBOR, as type: UInt16.Type, decoder: InternalValueDecoder<CBOR, Self>) throws -> UInt16? {
    switch value.untagged {
    case .double(let dbl): return try coerce(dbl, at: decoder.codingPath)
    case .float(let flt): return try coerce(flt, at: decoder.codingPath)
    case .unsignedInt(let uint): return try coerce(uint, at: decoder.codingPath)
  ...
  public static func unbox(_ value: CBOR, as type: UInt32.Type, decoder: InternalValueDecoder<CBOR, Self>) throws -> UInt32? {
    switch value.untagged {
    case .double(let dbl): return try coerce(dbl, at: decoder.codingPath)
    case .float(let flt): return try coerce(flt, at: decoder.codingPath)
    case .unsignedInt(let uint): return try coerce(uint, at: decoder.codingPath)
  ...
  public static func unbox(_ value: CBOR, as type: UInt64.Type, decoder: InternalValueDecoder<CBOR, Self>) throws -> UInt64? {
    switch value.untagged {
    case .double(let dbl): return try coerce(dbl, at: decoder.codingPath)
    case .float(let flt): return try coerce(flt, at: decoder.codingPath)
  ...
  public static func unbox(_ value: CBOR, as type: Float.Type, decoder: InternalValueDecoder<CBOR, Self>) throws -> Float? {
    switch value.untagged {
    case .double(let dbl): return try coerce(dbl, at: decoder.codingPath)
  ...
  public static func unbox(_ value: CBOR, as type: Double.Type, decoder: InternalValueDecoder<CBOR, Self>) throws -> Double? {
    switch value.untagged {
  ...
  public static func unbox(_ value: CBOR, as type: String.Type, decoder: InternalValueDecoder<CBOR, Self>) throws -> String? {
    switch value.untagged {
    case .null: return nil
  ...
  public static func unbox(_ value: CBOR, as type: UUID.Type, decoder: InternalValueDecoder<CBOR, Self>) throws -> UUID? {
    switch value {
    case .null: return nil
    case .utf8String(let string):
    ...
    case .tagged(.uuid, let tagged):
      guard case .byteString(let data) = tagged else {
        throw DecodingError._typeMismatch(at: decoder.codingPath, expectation: type, reality: tagged)
      }
      var uuid = UUID_NULL
      withUnsafeMutableBytes(of: &uuid) { ptr in
        _ = data.copyBytes(to: ptr)
  ...
  public static func unbox(_ value: CBOR, as type: Date.Type, decoder: InternalValueDecoder<CBOR, Self>) throws -> Date? {
  ...
  public static func unbox(_ value: CBOR, as type: Data.Type, decoder: InternalValueDecoder<CBOR, Self>) throws -> Data? {
    switch value.untagged {
    case .null: return nil
  ...
  public static func unbox(_ value: CBOR, as type: Decimal.Type, decoder: InternalValueDecoder<CBOR, Self>) throws -> Decimal? {
    guard !value.isNull else { return nil }
    let doubleValue = try unbox(value, as: Double.self, decoder: decoder)!
    return Decimal(doubleValue)
  }

  public static func valueToUnkeyedValues(_ value: CBOR, decoder: InternalValueDecoder<CBOR, Self>) throws -> [CBOR]? {
    guard case .array(let array) = value else { return nil }
    return array
  }

  public static func valueToKeyedValues(_ value: CBOR, decoder: InternalValueDecoder<CBOR, Self>) throws -> [String: CBOR]? {
